      ...
      passwordInput.addEventListener('input', () => {
        ...
        Object.keys(passReqs).forEach(key => {
          ...
          if (req.regex.test(value)) {
            req.element.textContent = "✅ " + req.element.textContent.slice(2); // Replace icon with check
          } else {
            req.element.textContent = "❌ " + req.element.textContent.slice(2); // Replace icon with cross
          }
        });
      });
    }
